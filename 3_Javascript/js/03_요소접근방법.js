...
function fnTest3(){
    ...
    for(let i=0 ; i<arr.length; i++){
        ...
        const num = arr[i].innerText;  // 숫자를 얻어와
...
function fnTest4(){
    ...
    // input 요소에 작성된 값을 얻어와 alert으로 추력
    alert(input.value);
...
function fnTest5(){
    ...
    for(let i = 0; i < hobby.length ; i++){
        ...
        // .checked 속성
        // 해당 요소가 체크 되어있따면 true / 아니면 false
...
function fnTest6(){
    ...
    // (여러 요소가 선택된 경우 첫번째 요소만 선택됨)
    document.querySelector("#cssTest").style.border ="3px solid blue";
...
function inputEnter(){
    ...
    // 키보드 입력 과정
    // 키누름 -> keydown 이벤트 -> input태그에 입력
    // -> keypress -> (손을 뗴면) keyup

 //   console.log(window.event.key); // 현재 눌러진 키 확인
    //keydown : 모든 키가 눌렸을 때
    //keypress : 영어/숫자/특수문자/띄어쓰기가 눌러지고 있dmfEo
